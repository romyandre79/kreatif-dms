  ...
  <div class="space-y-8 pb-10">
    ...
    <div v-motion-fade class="bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500 p-6 rounded-r-2xl flex items-center justify-between shadow-sm">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h3 class="font-black text-orange-900 dark:text-orange-200">Urgent Action Required</h3>
          <p class="text-sm text-orange-700/80 dark:text-orange-400/80">You have 12 pending requests that have exceeded the standard SLA time.</p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <div class="xl:col-span-2 glass rounded-2xl overflow-hidden" v-motion-slide-visible-bottom>
        <div class="px-8 py-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <h3 class="font-black text-lg text-slate-800 dark:text-white">Urgent Approval Queue</h3>
          <div class="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            <button class="px-4 py-1.5 text-xs font-bold rounded-lg bg-white dark:bg-slate-700 shadow-sm">Urgent</button>
            <button class="px-4 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-700 transition-colors">Normal</button>
            <button class="px-4 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-700 transition-colors">All</button>
          </div>
        </div>
          ...
          <table class="w-full text-left">
            <thead>
              <tr class="text-slate-400 text-[10px] uppercase font-black tracking-widest bg-slate-50/50 dark:bg-slate-900/50">
                <th class="px-8 py-4">Submission</th>
                <th class="px-8 py-4">Staff Name</th>
                <th class="px-8 py-4">Days Elapsed</th>
      ...
      <div class="space-y-8">
        ...
        <div class="glass rounded-3xl p-8 flex flex-col items-center justify-center text-center" v-motion-slide-visible-bottom>
          <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-8 self-start">Team SLA Compliance</h3>
          <div class="relative w-40 h-40 flex items-center justify-center">
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span class="text-[10px] font-bold text-slate-400 uppercase">Target 95%</span>
            </div>
          </div>
        </div>
        ...
        <div class="glass rounded-3xl p-8" v-motion-slide-visible-bottom>
          <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-8">Top Submitters</h3>
          <div class="space-y-6">
            <div v-for="(person, index) in topSubmitters" :key="index" class="flex items-center gap-4">
              ...
              <span class="text-xs font-black text-slate-800 dark:text-white">{{ person.count }} <span class="text-[10px] text-slate-400 ml-0.5">Docs</span></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
      ...
      <div class="glass rounded-2xl p-8" v-motion-slide-visible-bottom>
        <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-8">Quick Bulk Actions</h3>
      ...
      <div class="glass rounded-2xl p-8" v-motion-slide-visible-bottom>
        <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-8">Recent Decisions Log</h3>
        <div class="space-y-6">
          <div v-for="(log, index) in decisions" :key="index" class="flex gap-4">
            ...
            <div>
              ...
              <div class="flex items-center gap-3 mt-1.5">
                ...
                <span class="text-[10px] font-bold text-slate-400">Submitter: {{ log.submitter }}</span>
              </div>
            </div>
          </div>
        </div>
...
const stats = [
  { label: 'Priority Pending', value: '12', trend: '+15%', trendUp: true, progress: 65, barColor: 'bg-orange-500' },
  { label: 'Approved Today', value: '07', trend: '+2%', trendUp: true, progress: 40, barColor: 'bg-green-500' },
  { label: 'Team Submissions', value: '45', trend: '-5%', trendUp: false, progress: 80, barColor: 'bg-blue-600' },
  { label: 'Avg Approval Time', value: '2.3h', trend: '-0.4h', trendUp: true, progress: 30, barColor: 'bg-primary-500' },
]

const queue = [
  { 
    id: 'INV-2023-8821', desc: 'Marketing Budget Approval', staff: 'Budi Hartono', avatar: 'https://i.pravatar.cc/150?u=budi',
    elapsed: '4 days ago', status: 'OVERDUE', statusBg: 'bg-red-100 text-red-600', statusColor: 'text-red-500',
    bgClass: 'bg-red-50/50 dark:bg-red-900/10', borderColor: 'border-red-500'
  },
  { 
    id: 'DOC-XP-012', desc: 'Project Alpha Final Signoff', staff: 'Ani Wijaya', avatar: 'https://i.pravatar.cc/150?u=ani',
    elapsed: '3 days ago', status: 'CRITICAL', statusBg: 'bg-red-100 text-red-600', statusColor: 'text-red-500',
    bgClass: 'bg-red-50/50 dark:bg-red-900/10', borderColor: 'border-red-400'
  },
  { 
    id: 'PR-990-2', desc: 'Software License Renewal', staff: 'Joko Susilo', avatar: 'https://i.pravatar.cc/150?u=joko',
    elapsed: '1 day ago', status: 'PENDING', statusBg: 'bg-blue-100 text-blue-600', statusColor: 'text-slate-500',
    bgClass: '', borderColor: 'border-transparent'
  },
]

const topSubmitters = [
  { rank: '01', name: 'Indra Pratama', count: '12', avatar: 'https://i.pravatar.cc/150?u=1' },
  { rank: '02', name: 'Maya Putri', count: '10', avatar: 'https://i.pravatar.cc/150?u=2' },
  { rank: '03', name: 'Dedi Kurnia', count: '8', avatar: 'https://i.pravatar.cc/150?u=3' },
]

const bulkActions = [
  { label: 'Bulk Approve', desc: 'Verify all valid small docs', icon: LucideLayoutGrid },
  { label: 'Deep Analytics', desc: 'View performance trends', icon: LucideBarChart3 },
  { label: 'Archive Log', desc: 'Quarterly document export', icon: LucideArchive },
  { label: 'Help Desk', desc: 'IT & System Support', icon: LucideLifeBuoy },
]

const decisions = [
  { action: 'Approved', target: 'Travel Request #TR-2201', time: '20 mins ago', submitter: 'Kevin S.', icon: LucideCheckCircle2, color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-900/20' },
  { action: 'Rejected', target: 'Requisition #RQ-882 (Insufficient Funds)', time: '1 hour ago', submitter: 'Linda R.', icon: LucideXCircle, color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-900/20' },
  { action: 'Commented', target: 'on Purchase Order #PO-991', time: '3 hours ago', submitter: 'Finance', icon: LucideMessageSquare, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' },
]
</script>
