<template>
  <div class="space-y-8 pb-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4" v-motion-fade>
      <div>
        <h1 class="text-3xl font-bold text-[#1E3A5F] dark:text-white">Welcome, {{ user?.full_name || 'User' }}</h1>
        <p class="text-slate-500 font-bold text-sm mt-1">Enterprise Dashboard • Central Document Control</p>
      </div>
      <div class="flex items-center gap-3">
        ...
        <div class="bg-slate-100 dark:bg-slate-800 px-4 py-2 rounded-xl flex items-center gap-2 border border-slate-200 dark:border-slate-700">
          ...
          <span class="text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-tighter">Storage 72%</span>
        </div>
        <div class="bg-orange-50 dark:bg-orange-900/20 px-4 py-2 rounded-xl flex items-center gap-2 border border-orange-100 dark:border-orange-800">
          ...
          <span class="text-xs font-black text-orange-700 dark:text-orange-400 uppercase tracking-tighter">Active Tasks: 31</span>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div v-motion-slide-visible-bottom class="glass p-6 rounded-2xl">
        <div class="flex items-start justify-between mb-4">
          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Inbound Pipeline</p>
          <LucideArrowRightCircle class="w-5 h-5 text-slate-300" />
        </div>
        <p class="text-3xl font-black text-slate-900 dark:text-white">1,248 Units</p>
        <p class="text-xs font-bold text-green-500 mt-2">+12% vs last week</p>
      </div>

      <!-- Loan Operations -->
      <div v-motion-slide-visible-bottom class="glass p-6 rounded-2xl">
        <div class="flex items-start justify-between mb-4">
          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Loan Operations</p>
          <LucidePackageCheck class="w-5 h-5 text-orange-400" />
        </div>
        <p class="text-3xl font-black text-slate-900 dark:text-white">42 Active</p>
        <p class="text-xs font-bold text-red-500 mt-2">⚠ 8 Overdue Items</p>
      </div>
      ...
      <div v-motion-slide-visible-bottom class="glass p-6 rounded-2xl flex items-center gap-6">
        ...
        <div>
          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">WH. Capacity</p>
      ...
      <div v-motion-slide-visible-bottom class="glass p-6 rounded-2xl">
        ...
        <p class="text-2xl font-black text-slate-900 dark:text-white">All Nominal</p>
        <p class="text-[10px] font-bold text-slate-400 mt-1 uppercase">99.9% Uptime across nodes</p>
      </div>
    </div>
    ...
    <div class="glass rounded-2xl overflow-hidden" v-motion-slide-visible-bottom>
      <div class="px-8 py-6 flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-black text-lg text-[#1E3A5F] dark:text-white">Priority Operation Queue</h3>
        </div>
        <button class="text-primary-600 font-black text-sm hover:underline">View All Queue</button>
      </div>
        ...
        <table class="w-full text-left">
          <thead>
            <tr class="text-slate-400 text-[10px] uppercase font-black tracking-widest bg-slate-50/50 dark:bg-slate-900/50">
              ...
              <th class="px-8 py-4">Client / Dept</th>
              <th class="px-8 py-4">Type</th>
              <th class="px-8 py-4">SLA Status</th>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <div class="xl:col-span-2 glass rounded-2xl p-8" v-motion-slide-visible-bottom>
        <h3 class="font-black text-xl text-[#1E3A5F] dark:text-white mb-8">Today's Activity Feed</h3>
        <div class="space-y-8">
      ...
      <div class="glass rounded-2xl p-8" v-motion-slide-visible-bottom>
        <div class="flex items-center justify-between mb-8">
          <h3 class="font-black text-xl text-[#1E3A5F] dark:text-white">Alerts & Warnings</h3>
          <span class="bg-red-500 text-white text-[9px] font-black uppercase px-2 py-1 rounded-md">6 Action Required</span>
        </div>
        <div class="space-y-4">
          ...
          <div class="flex items-center gap-4 p-5">
            ...
            <div>
              <p class="text-sm font-black text-slate-700 dark:text-slate-300">Sync Delay (Node Jakarta)</p>
              <p class="text-[11px] text-slate-400 font-bold leading-relaxed">Database synchronization delay is currently 12ms above normal threshold.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="glass rounded-2xl p-10" v-motion-slide-visible-bottom>
      <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-10">Quick Access Tools</h3>
...
const queueTabs = [
  { label: 'All Tasks', count: 31, active: true },
  { label: 'Receive', count: 12, active: false },
  { label: 'Scan', count: 8, active: false },
  { label: 'Metadata', count: 6, active: false },
  { label: 'Loan Prep', count: 5, active: false },
]

const queueItems = [
  { id: '#REQ-2023-0891', client: 'Legal Department', dept: 'Confidential Contracts', type: 'Bulk Scan', sla: 'OVERDUE 2H', slaBg: 'bg-red-100 text-red-600', btnLabel: 'Start Scanning', btnClass: 'bg-[#1E3A5F] text-white hover:bg-[#152943]' },
  { id: '#PKG-4501-A', client: 'Corporate Finance', dept: 'Physical Invoices', type: 'Receipt', sla: 'DUE IN 45M', slaBg: 'bg-orange-100 text-orange-600', btnLabel: 'Receive Package', btnClass: 'bg-blue-100 text-blue-700 hover:bg-blue-200' },
  { id: '#MTD-8821-C', client: 'Human Resources', dept: 'Employee Records', type: 'Indexing', sla: 'ON SCHEDULE', slaBg: 'bg-slate-100 text-slate-500', btnLabel: 'Apply Metadata', btnClass: 'bg-blue-100 text-blue-700 hover:bg-blue-200' },
]

const activities = [
  { text: '<span class="font-black">System:</span> Automated OCR Batch #772 Completed', sub: '14,201 pages indexed successfully', time: '2 mins ago', dot: 'bg-blue-500' },
  { text: '<span class="font-black">User Budi S.</span> received Physical Box #BX-902', sub: 'Warehouse Section D-12', time: '15 mins ago', dot: 'bg-green-500' },
  { text: '<span class="font-black">Loan Request #LR-202</span> Pending Approval', sub: 'Requester: Diana (Marketing)', time: '45 mins ago', dot: 'bg-orange-500' },
  { text: '<span class="font-black">Security Log:</span> Admin Login detected', sub: 'IP 192.168.1.45', time: '1 hour ago', dot: 'bg-slate-300' },
]
...
const tools = [
  { label: 'Label Printing', icon: LucidePrinter },
  { label: 'Quick Scan', icon: LucideScan },
  { label: 'Audit Logs', icon: LucideClipboardList },
  { label: 'Inventory Map', icon: LucideMap },
  { label: 'User Access', icon: LucideUsers },
  { label: 'IT Support', icon: LucideHelpCircle },
]

const footerStats = [
  { label: 'AVG THROUGHPUT', value: '4.2k pgs/hr' },
  { label: 'AVG TURNAROUND', value: '5.8 hrs' },
  { label: 'OCR ACCURACY', value: '99.2%' },
  { label: 'SLA SATISFACTION', value: '94.8%' },
]
</script>
