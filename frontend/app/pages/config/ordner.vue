                  ...
                  >
                    ...
                    <td class="p-4">
                      <div v-if="row.editing && row.isNew" class="max-w-xs">
                        <select v-model="row.box_id" class="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500/20">
                          <option value="" disabled>{{ $t('admin.config.ordner.select_box') }}</option>
                          <option v-for="box in boxes" :key="box.id" :value="box.id">
                            {{ box.name }} ({{ box.rack_name }})
                          </option>
                        </select>
                      </div>
      ...
      <aside class="w-80 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 flex flex-col p-8 overflow-y-auto custom-scrollbar">
        <section class="space-y-8">
          ...
          <div class="space-y-6">
            ...
            <div class="p-6 rounded-2xl border-l-4 border-red-500 bg-red-50/50 dark:bg-red-900/10 space-y-3 shadow-sm">
...
const fetchData = async () => {
  ...
  try {
    const [ordnersRes, boxesRes] = await Promise.all([
      $fetch(`${config.public.apiBase}/master/ordners`, {
...
const deleteRow = async (id) => {
  ...
  try {
    await $fetch(`${config.public.apiBase}/master/ordners/${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${auth.accessToken}` }
    })
    fetchData()
