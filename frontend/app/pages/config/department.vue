            ...
            <div class="px-8 py-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/30 dark:bg-slate-900/30">
              <div class="relative w-96 group">
                ...
                <input 
                  type="text" 
                  v-model="searchQuery"
                  :placeholder="$t('admin.config.dept.filter_placeholder')" 
              ...
              <table class="w-full text-left border-collapse">
                <thead>
                  <tr class="bg-slate-50/50 dark:bg-slate-900/50 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50 dark:border-slate-800">
                    <th class="p-4 pl-8">{{ $t('admin.config.dept.table.name') }}</th>
                    <th class="p-4">{{ $t('admin.config.dept.table.branch') }}</th>
                    <th class="p-4">{{ $t('admin.config.dept.table.head') }}</th>
                ...
                <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                  <tr v-if="filteredDepartments.length === 0" class="hover:bg-transparent">
                    ...
                    <td class="p-4 pl-8">
                      <div v-if="row.editing" class="max-w-md">
                        <input 
                          ...
                          :placeholder="$t('admin.config.dept.table.name')"
                        />
                      </div>
            ...
            <div class="px-10 py-2.5 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between relative z-0">
              <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2 ml-4">
                  <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $t('admin.config.dept.rows') }}:</span>
        ...
        <section class="space-y-8">
          ...
          <div class="space-y-6">
            <div class="p-6 rounded-2xl border-l-4 border-blue-500 bg-blue-50/50 dark:bg-blue-900/10 space-y-3 shadow-sm">
              <h4 class="text-[11px] font-black text-[#1E3A5F] dark:text-white uppercase tracking-tight">Hierarki Organisasi</h4>
              <p class="text-[10px] font-bold text-slate-500 leading-relaxed">Setiap departemen wajib terhubung dengan satu cabang (branch) aktif.</p>
            </div>
            <div class="p-6 rounded-2xl border-l-4 border-amber-500 bg-amber-50/50 dark:bg-amber-900/10 space-y-3 shadow-sm">
              <h4 class="text-[11px] font-black text-[#1E3A5F] dark:text-white uppercase tracking-tight">Penanggung Jawab</h4>
              <p class="text-[10px] font-bold text-slate-500 leading-relaxed">Kepala departemen bertanggung jawab atas persetujuan peminjaman dokumen di lingkup departemennya.</p>
            </div>
          </div>
        </section>
      </aside>
...
const fetchData = async () => {
  ...
  try {
    ...
    error.value = err.data?.message || t('admin.config.dept.error_fetch')
  } finally {
    loading.value = false
  }
}
...
const getHeadName = (headId) => {
  ...
  return user ? user.full_name : t('admin.config.dept.user_not_found')
}
...
const cancelEdit = (row) => {
  if (row.isNew) {
    departments.value = departments.value.filter(d => d !== row)
  } else {
    row.editing = false
    fetchData()
  }
...
const saveRow = async (row) => {
  if (!row.name.trim()) {
    error.value = t('admin.config.dept.error_name')
    return
  }
  if (!row.branch_id) {
    error.value = t('admin.config.dept.error_branch')
    return
  }
  ...
  try {
    ...
    error.value = err.data?.message || t('admin.config.dept.error_save')
  }
}
...
const paginatedDepartments = computed(() => filteredDepartments.value.slice(startIndex.value, endIndex.value))
