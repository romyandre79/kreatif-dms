  ...
  <div class="max-w-7xl mx-auto space-y-6 pb-24">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
            Status semua dokumen yang Anda ajukan
          </p>
        </div>
        <button
          @click="navigateTo('/documents/upload')"
          class="flex items-center gap-2 px-5 py-2.5 bg-[#1E3A5F] hover:bg-[#152943] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-900/20 transition-all"
        >
    ...
    <div class="glass rounded-3xl overflow-hidden" v-motion-fade>
      ...
      <div v-else-if="items.length === 0" class="py-16 flex flex-col items-center justify-center gap-3">
        <LucideFileClock class="w-12 h-12 text-slate-200 dark:text-slate-700" />
        <p class="text-sm font-black text-slate-400 uppercase tracking-widest">Belum ada pengajuan</p>
        <button
          @click="navigateTo('/documents/upload')"
          class="mt-2 px-6 py-2.5 bg-[#1E3A5F] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#152943] transition-all"
        >
      ...
      <table v-else class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-700">
            ...
            <th class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tanggal Kirim</th>
            <th class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th>
            <th class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Diproses Oleh</th>
            <th class="pr-5 py-3 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Aksi</th>
          </tr>
        </thead>
          ...
          >
            ...
            <td class="px-5 py-3.5">
              <div class="space-y-0.5">
                <p class="text-xs font-black text-[#1E3A5F] dark:text-white leading-tight">{{ item.title }}</p>
            ...
            <td class="px-4 py-3.5">
              <span class="text-[9px] font-black text-slate-500 uppercase tracking-widest">{{ item.type_name || '—' }}</span>
            </td>
            ...
            <td class="px-4 py-3.5">
              <div class="space-y-1">
                ...
                <p v-if="item.approval_note && item.status === 'rejected'" class="text-[8px] font-medium text-red-400 leading-tight line-clamp-1 max-w-[160px]">
                  {{ item.approval_note }}
                </p>
              </div>
            </td>
            ...
            <td class="pr-5 py-3.5 text-right">
              <div class="flex items-center gap-2 justify-end">
                ...
                <button
                  ...
                  @click="navigateTo(`/documents/upload?id=${item.id}`)"
                  class="px-3 py-1.5 bg-amber-50 text-amber-600 border border-amber-100 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-amber-100 transition-all"
                >
                  Edit
                </button>
                <button
                  @click="navigateTo(`/documents/${item.id}`)"
                  class="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-white transition-all flex items-center gap-1"
                >
                  Detail
...
<script setup>
import {
  LucidePlus, LucideSearch, LucideChevronDown,
  LucideChevronLeft, LucideChevronRight, LucideArrowRight, LucideFileClock
...
const statusCounts = ref({ all: 0, pending: 0, approved: 0, rejected: 0, active: 0 })
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await $api(`/documents/my-submissions?${params}`)
    if (res?.data) {
      items.value = res.data.items || []
      pagination.value = { ...pagination.value, ...res.data.pagination }
    }
  } catch (e) {
    console.error('[MySubmissions] Failed:', e)
    items.value = []
  } finally {
...
const loadCounts = async () => {
  ...
  const results = await Promise.allSettled(
    statuses.map(s => $api(`/documents/my-submissions?limit=1${s ? `&status=${s}` : ''}`))
  )
  results.forEach((r, i) => {
