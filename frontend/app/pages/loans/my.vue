<template>
  <div class="max-w-[1600px] mx-auto space-y-6 pb-20">
    ...
    <Transition name="fade">
      <div v-if="submissionSuccess" class="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center justify-between gap-4 text-emerald-700 font-bold text-sm shadow-sm" v-motion-slide-top>
    ...
    <!-- Filters/Tabs -->
    <div class="flex items-center gap-12 border-b border-slate-100 pb-1" v-motion-fade>
    ...
    <div class="glass rounded-3xl bg-white border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden" v-motion-fade>
        ...
        <table class="w-full text-left min-w-[700px]">
          <thead>
            <tr class="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
              <th class="px-8 py-5">{{ $t('loans.my.table.req_no') }}</th>
              <th class="px-8 py-5">{{ $t('loans.my.table.total_docs') }}</th>
              <th class="px-8 py-5">{{ $t('loans.my.table.checkout_date') }}</th>
              <th class="px-8 py-5">{{ $t('loans.my.table.due_date') }}</th>
              <th class="px-8 py-5">{{ $t('loans.my.table.remaining_days') }}</th>
              <th class="px-8 py-5">{{ $t('loans.my.table.status') }}</th>
              <th class="px-8 py-5"></th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-50">
            <tr v-if="filteredLoans.length === 0">
              <td colspan="7" class="px-8 py-16 text-center text-sm font-bold text-slate-400 italic">
                {{ isLoading ? 'Loading...' : 'Tidak ada data peminjaman.' }}
              </td>
            </tr>
            <tr v-for="loan in filteredLoans" :key="loan.no"
                @click="openDrawer(loan)"
                :class="`group cursor-pointer transition-all ${selectedLoan?.no === loan.no ? 'bg-primary-50/30' : 'hover:bg-slate-50/50'}`">
              <td class="px-8 py-6">
                <div class="flex items-center gap-3">
                  <div :class="`w-3.5 h-3.5 rounded-full border-2 transition-all flex items-center justify-center shrink-0 ${selectedLoan?.no === loan.no ? 'border-[#1E3A5F]' : 'border-slate-200'}`">
              ...
              <td class="px-8 py-6 text-xs font-bold text-slate-500 uppercase">{{ $t('loans.my.table.docs_count', { count: loan.docsCount }) }}</td>
              <td class="px-8 py-6 text-xs font-bold text-slate-500 uppercase">{{ loan.checkoutDate }}</td>
              <td class="px-8 py-6 text-xs font-bold text-slate-500 uppercase">{{ loan.dueDate }}</td>
              <td class="px-8 py-6">
                <span :class="`px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${loan.remainingColor}`">
                  {{ loan.remainingText }}
                </span>
              </td>
              <td class="px-8 py-6">
              ...
              <td class="px-8 py-6">
                <LucideChevronRight class="w-4 h-4 text-slate-300 group-hover:text-[#1E3A5F] transition-colors" />
              </td>
            </tr>
          </tbody>
        </table>
  ...
  <!-- Drawer Overlay -->
  <Transition name="fade">
    <div v-if="drawerOpen" @click="closeDrawer" class="fixed inset-0 bg-[#1E3A5F]/30 backdrop-blur-sm z-[150]"></div>
  </Transition>

  <!-- Detail Drawer -->
  <Transition name="drawer">
    <div v-if="drawerOpen"
         class="fixed top-0 right-0 h-full w-[440px] bg-white border-l border-slate-200/60 shadow-2xl z-[160] flex flex-col overflow-hidden">
      ...
      <div class="flex items-center justify-between px-8 py-6 border-b border-slate-100 shrink-0">
        ...
        <button @click="closeDrawer" class="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar">
        <div class="p-8 space-y-8">
          ...
          <div class="p-6 bg-orange-50/50 border border-orange-100 rounded-2xl text-center space-y-3">
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-0 relative">
              ...
              <div class="relative flex items-start gap-5 pb-8">
                <div v-if="getStepState('approved') === 'completed'" class="relative z-10 w-9 h-9 rounded-full bg-[#1E3A5F] text-white flex items-center justify-center shadow-lg shadow-blue-900/10 shrink-0">
                  <LucideCheck class="w-4 h-4" />
                </div>
                <div v-else-if="getStepState('approved') === 'current'" class="relative z-10 w-9 h-9 rounded-full bg-white border-2 border-[#1E3A5F] flex items-center justify-center shadow-lg shrink-0">
                  <div class="w-2 h-2 bg-[#1E3A5F] rounded-full animate-pulse"></div>
                </div>
                <div v-else-if="getStepState('approved') === 'rejected'" class="relative z-10 w-9 h-9 rounded-full bg-red-500 text-white flex items-center justify-center shadow-lg shrink-0">
                  <LucideX class="w-4 h-4" />
                </div>
                <div v-else class="relative z-10 w-9 h-9 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center shadow-lg shrink-0">
                  <div class="w-1.5 h-1.5 bg-slate-300 rounded-full"></div>
                </div>
                <div>
                  <h4 :class="['text-sm uppercase tracking-tight', getStepState('approved') === 'completed' || getStepState('approved') === 'current' ? 'text-[#1E3A5F] font-black' : '', getStepState('approved') === 'rejected' ? 'text-red-500 font-black' : '', getStepState('approved') === 'upcoming' ? 'text-slate-300 font-bold' : '']">{{ $t('loans.my.detail.steps.approved') }}</h4>
              ...
              <!-- Ready -->
              <div class="relative flex items-start gap-5 pb-8">
                <div v-if="getStepState('ready') === 'completed'" class="relative z-10 w-9 h-9 rounded-full bg-[#1E3A5F] text-white flex items-center justify-center shadow-lg shadow-blue-900/10 shrink-0">
                  <LucideCheck class="w-4 h-4" />
                </div>
                <div v-else-if="getStepState('ready') === 'current'" class="relative z-10 w-9 h-9 rounded-full bg-white border-2 border-[#1E3A5F] flex items-center justify-center shadow-lg shrink-0">
                  <div class="w-2 h-2 bg-[#1E3A5F] rounded-full animate-pulse"></div>
                </div>
                <div v-else class="relative z-10 w-9 h-9 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center shadow-lg shrink-0">
                ...
                <div>
                  <h4 :class="['text-sm uppercase tracking-tight', getStepState('ready') === 'completed' || getStepState('ready') === 'current' ? 'text-[#1E3A5F] font-black' : 'text-slate-300 font-bold']">{{ $t('loans.my.detail.steps.ready') }}</h4>
                  <p class="text-[10px] font-bold text-slate-400 uppercase mt-1">{{ selectedLoan?.readyDate || '-' }}</p>
                </div>
              </div>

              <!-- On Loan -->
              <div :class="`relative flex items-start gap-5 ${selectedLoan?.status === 'EXTENSION PENDING' ? 'pb-8' : ''}`">
                <div v-if="getStepState('on_loan') === 'completed'" class="relative z-10 w-9 h-9 rounded-full bg-emerald-500 text-white flex items-center justify-center shadow-lg shadow-emerald-500/10 shrink-0">
                  <LucideCheck class="w-4 h-4" />
                </div>
                <div v-else-if="getStepState('on_loan') === 'current'" class="relative z-10 w-9 h-9 rounded-full bg-white border-2 border-[#1E3A5F] flex items-center justify-center shadow-lg shrink-0">
                ...
                <div>
                  <h4 :class="['text-sm uppercase tracking-tight', getStepState('on_loan') === 'completed' ? 'text-emerald-500 font-black' : '', getStepState('on_loan') === 'current' ? 'text-[#1E3A5F] font-black' : '', getStepState('on_loan') === 'upcoming' ? 'text-slate-300 font-bold' : '']">{{ $t('loans.my.detail.steps.on_loan') }}</h4>
              ...
              <div v-if="selectedLoan?.status === 'EXTENSION PENDING'" class="relative flex items-start gap-5">
                <div class="relative z-10 w-9 h-9 rounded-full bg-white border-2 border-orange-400 flex items-center justify-center shadow-lg shrink-0">
          ...
          <div class="space-y-3 pt-6 border-t border-slate-50">
            <div class="flex gap-3 items-start p-4 bg-slate-50/50 rounded-2xl">
            ...
            <div class="flex gap-3 items-start p-5 bg-red-50 border border-red-100 rounded-2xl">
              <LucideAlertTriangle class="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
              <div class="space-y-1">
                <p class="text-[10px] font-black text-red-600 uppercase tracking-widest">{{ $t('loans.my.detail.policy.penalty_title') }}</p>
      ...
      <!-- Drawer Footer CTA -->
      <div v-if="canReschedule" class="shrink-0 p-6 border-t border-slate-100">
        <div v-if="selectedLoan?.status === 'EXTENSION PENDING'" class="p-5 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
          <h3 class="text-sm font-black text-slate-700 uppercase tracking-tight">{{ $t('loans.my.detail.ext_pending.title') }}</h3>
          <p class="text-xs font-medium text-slate-500 leading-relaxed">{{ $t('loans.my.detail.ext_pending.desc') }}</p>
        </div>
        <div v-else class="p-5 bg-[#1E3A5F] rounded-2xl text-white shadow-xl shadow-blue-900/20 space-y-4">
          <div class="space-y-1">
            <h3 class="text-sm font-black uppercase tracking-tight">{{ $t('loans.my.detail.ext_promo.title') }}</h3>
            <p class="text-[11px] font-medium text-blue-200/80 leading-relaxed">{{ $t('loans.my.detail.ext_promo.desc') }}</p>
          </div>
          <button @click="showExtensionModal = true" class="w-full py-4 bg-white text-[#1E3A5F] rounded-xl text-xs font-black uppercase tracking-[0.2em] shadow-lg hover:bg-blue-50 transition-all">
  ...
  <Transition name="scale">
    <div v-if="showExtensionModal" class="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-[#1E3A5F]/40 backdrop-blur-sm">
      <div class="glass max-w-lg w-full bg-white rounded-3xl p-10 shadow-2xl border border-white space-y-8" v-motion-pop>
        <!-- Modal Header -->
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-3">
            ...
            <textarea v-model="extensionReason"
                      ...
                      rows="4"
                      class="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium text-slate-600 outline-none focus:ring-4 focus:ring-primary-500/5 focus:border-primary-500 transition-all resize-none"></textarea>
          </div>

          <!-- Info Box -->
          <div class="p-5 bg-blue-50/50 border border-blue-100 rounded-2xl flex gap-4 items-start">
            <LucideInfo class="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
            <p class="text-[11px] font-bold text-slate-500 leading-relaxed">{{ $t('loans.my.modal.info_box') }}</p>
          </div>
        </div>
...
import {
  LucideHistory, LucideCheck, LucideInfo, LucideAlertTriangle, LucideX, LucideSend, LucideChevronRight
