<template>
  <div class="max-w-7xl mx-auto space-y-5 pb-16">
    ...
    <!-- Title & Status Badges -->
    <div class="flex flex-wrap items-end justify-between gap-4" v-motion-fade>
      <div class="space-y-0.5">
        ...
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ doc?.reg_no || doc?.id || route.params.id }}</p>
      </div>
      <div class="flex items-center gap-2">
        <span class="px-3 py-1.5 bg-orange-50 text-orange-600 border border-orange-100 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm">
          {{ $t('approvals.detail.status.pending') || 'Pending Your Approval' }}
        </span>
        <span v-if="doc?.urgency === 'urgent' || true" class="px-3 py-1.5 bg-red-50 text-red-600 border border-red-100 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div class="lg:col-span-2 space-y-5">
        ...
        <div class="glass rounded-2xl p-6 space-y-6" v-motion-fade>
          <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">{{ $t('approvals.detail.sections.submission') }}</h3>
          
          <div class="grid grid-cols-2 gap-x-6 gap-y-4">
            <!-- Row 1 -->
            <div class="space-y-1">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.list.table.reg_id') || 'ID REGISTRASI' }}</p>
              <p class="text-xs font-black text-[#1E3A5F] font-mono">{{ doc?.id || doc?.entity_id || '---' }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.detail.fields.submitted_by') }}</p>
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-slate-100 overflow-hidden ring-2 ring-white shadow-md">
                  <img :src="`https://ui-avatars.com/api/?name=${doc?.owner_name || 'U'}&background=random`" class="w-full h-full object-cover" />
                </div>
                <p class="text-xs font-black text-[#1E3A5F]">{{ doc?.owner_name || '---' }}</p>
              </div>
            </div>

            <!-- Row 2 -->
            <div class="space-y-1 col-span-2">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.detail.fields.doc_info') }}</p>
              <p class="text-xs font-black text-[#1E3A5F]">{{ doc?.description || doc?.title || '---' }}</p>
            </div>

            <!-- Row 3 -->
            <div class="space-y-1.5">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.detail.fields.category') }}</p>
              <span class="px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-[9px] font-black uppercase tracking-widest border border-blue-100 inline-block">
                {{ doc?.type_name || '---' }}
              </span>
            </div>
            <div class="space-y-1">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.detail.fields.department') }}</p>
              <p class="text-xs font-black text-[#1E3A5F]">{{ doc?.department_name || '---' }}</p>
            </div>

            <!-- Row 4 -->
            <div class="space-y-1">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.detail.fields.date') }}</p>
              <p class="text-xs font-black text-[#1E3A5F]">{{ doc?.created_at ? new Date(doc.created_at).toLocaleDateString() : '---' }}</p>
            </div>
            <div class="space-y-1">
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('approvals.detail.fields.count') }}</p>
              <p class="text-xs font-black text-[#1E3A5F]">{{ doc?.metadata?.page_count || 1 }} {{ $t('approvals.detail.fields.files') }}</p>
            </div>
          </div>

          <!-- Notes Box -->
          <div v-if="doc?.description" class="bg-amber-50/50 border-l-4 border-amber-400 p-4 rounded-r-xl space-y-1">
            <p class="text-[9px] font-black text-amber-600 uppercase tracking-widest">{{ $t('approvals.detail.fields.notes') }}</p>
          ...
          <div v-if="doc?.batch_id" class="bg-slate-50 border border-slate-100 p-4 rounded-2xl flex items-center justify-between group cursor-pointer hover:bg-white transition-all shadow-sm">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm text-slate-400 group-hover:text-primary-600 transition-colors">
                <LucideLayers class="w-5 h-5" />
              </div>
              <div>
                <p class="text-xs font-black text-slate-700 uppercase tracking-tight">Part of Bulk Upload</p>
                <p class="text-[9px] text-slate-400 font-bold mt-0.5 uppercase tracking-widest">This submission belongs to Batch #{{ doc.batch_id }}</p>
              </div>
            </div>
            <button @click="navigateTo(`/approvals?batch=${doc.batch_id}`)" class="text-[9px] font-black text-primary-600 uppercase tracking-widest flex items-center gap-1.5 group-hover:gap-2 transition-all">
              View Batch
        ...
        <!-- Attachments List -->
        <div class="glass rounded-2xl p-6 space-y-5" v-motion-fade>
          <div class="flex items-center justify-between">
            <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">{{ $t('approvals.detail.sections.attachments') }} ({{ attachedFiles.length }})</h3>
          </div>

          <div class="space-y-3">
            <div v-for="file in attachedFiles" :key="file.id" class="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-xl hover:border-primary-200 hover:shadow-md transition-all group">
              <div class="flex items-center gap-4">
                <div class="w-10 h-10 bg-red-50 rounded-xl flex items-center justify-center text-red-500 shadow-sm">
                  <LucideFileText class="w-5 h-5" />
                </div>
                <div>
                  <div class="flex items-center gap-2">
                    <p class="text-xs font-black text-[#1E3A5F] uppercase tracking-tight">{{ file.file_name || file.filename || 'Document File' }}</p>
                  ...
                  <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ ((file.file_size || file.size || 0) / 1024).toFixed(1) }} KB • {{ file.mime_type || '—' }}</p>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <button @click="previewFile(file)" class="px-3.5 py-1.5 bg-slate-50 text-slate-600 rounded-md text-[9px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all border border-slate-100">Preview</button>
              </div>
            </div>
            <div v-if="!attachedFiles.length" class="text-center py-6">
              <LucideFileStack class="w-8 h-8 text-slate-200 mx-auto mb-2" />
              <p class="text-[10px] font-bold text-slate-300 uppercase tracking-widest">No attachments found</p>
            </div>
          </div>
        </div>

        <!-- Manager Notes Input -->
        <div class="glass rounded-2xl p-6 space-y-4" v-motion-fade>
          <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">{{ $t('approvals.detail.sections.manager_notes') }}</h3>
          <textarea 
            v-model="managerNotes"
            rows="3"
            class="w-full bg-slate-50 border border-slate-100 rounded-xl p-4 text-xs font-medium focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all outline-none"
      ...
      <div>
        <div class="glass rounded-2xl p-6 space-y-6" v-motion-fade>
          <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">{{ $t('approvals.detail.sections.workflow') }}</h3>
          
          <div class="relative pl-8 space-y-6">
            <!-- Vertical Line -->
            <div class="absolute left-[0.9rem] top-2 bottom-2 w-0.5 bg-slate-100"></div>

            <!-- Steps -->
            <div class="relative">
              <div class="absolute -left-[1.4rem] top-0 w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white ring-4 ring-white shadow-sm z-10">
                <LucideCheck class="w-3.5 h-3.5" />
              </div>
              <div class="space-y-0.5">
                <p class="text-xs font-black text-[#1E3A5F]">{{ $t('approvals.detail.workflow.submitted') }}</p>
            ...
            <div class="relative">
              <div class="absolute -left-[1.4rem] top-0 w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-white ring-4 ring-white shadow-sm z-10 animate-pulse">
                <LucideClock class="w-3.5 h-3.5" />
              </div>
              <div class="space-y-0.5">
                <p class="text-xs font-black text-orange-600">{{ $t('approvals.detail.workflow.pending_manager') }}</p>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
                  Assigned to You
                </p>
              </div>
            ...
            <div class="relative opacity-30">
              <div class="absolute -left-[1.4rem] top-0 w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-white ring-4 ring-white shadow-sm z-10">
                <span class="text-[9px] font-black">3</span>
              </div>
              <div class="space-y-0.5">
                <p class="text-xs font-black text-slate-800">{{ $t('approvals.detail.workflow.compliance') }}</p>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Not started</p>
              </div>
            </div>
            ...
            <div class="relative opacity-30">
              ...
              <div class="space-y-0.5">
                <p class="text-xs font-black text-slate-800">{{ $t('approvals.detail.workflow.archiving') }}</p>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Pending approval chain</p>
              </div>
            </div>
          </div>
        </div>
...
import { 
  LucideLayers, LucideArrowRight, LucideFileText, 
